...
static void rds_ring_poll_peer(struct rds_ep *ep, struct rds_peer *peer)
{
	...
	/* Make sure a lagging sender eventually sees progress even if the
	 * batch threshold was not reached and there is no reverse traffic. */
	if (drained && peer->rx_head != peer->rx_credit_sent)
		rds_ring_return_credit(ep, peer);
}
